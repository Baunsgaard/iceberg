...
import org.apache.iceberg.deletes.PositionDeleteRangeConsumer.RangeAccumulator;
...
public final class VectorizedPositionDeleteReader {
  ...
  public static PositionDeleteIndex read(
    ...
    RangeAccumulator coalescer = new RangeAccumulator(index);
   ...
   */
  private static void appendAll(CloseableIterator<ColumnarBatch> it, RangeAccumulator coalescer) {
    long[] buffer = null;
    while (it.hasNext()) {
  ...
  private static void appendFiltered(
      CloseableIterator<ColumnarBatch> it, CharSequence dataLocation, RangeAccumulator coalescer) {
  ...
  private static void appendGroupedByPath(
    ...
    RangeAccumulator coalescer = null;
    long[] buffer = null;
      ...
      try (ColumnarBatch batch = it.next()) {
        ...
        while (cursor < rows) {
          if (currentPath == null || !matches(pathVec, pathBuf, cursor, currentPath)) {
            ...
            coalescer = new RangeAccumulator(existing);
          }
