...
public class BenchmarkPositionDeleteRangeConsumer {
  ...
  /**
   * Exactly {@code count} distinct sorted positions uniformly sampled from a domain of {@code count
   * / density} slots, so the resulting density matches the requested value. This makes the inner
   * {@code pos - lastPosition == 1} branch track the requested density: SPARSE_50 is a true ~50/50
   * coin flip, SPARSE_5 is adversarial (~5% hits), SPARSE_95 is best-case (~95% hits).
   *
   * <p>Uses rejection sampling for the sparse half (density <= 0.5) and the complement (sampling
   * excluded positions) for the dense half to stay fast at high densities.
   */
